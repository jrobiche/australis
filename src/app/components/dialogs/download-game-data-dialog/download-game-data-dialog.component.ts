...
export class DownloadGameDataDialogComponent implements OnDestroy {
  ...
  async onDownloadDatabasesClick() {
    ...
    try {
      ...
      await this.#auroraFtpService.downloadAuroraDatabases(
  ...
  async onDownloadGameDataClick() {
    this.isDownloadInProgress = true;
    try {
      let dirs = await this.#auroraFtpService.listAuroraGameDataDirectories(
        this.gameConsoleConfiguration,
      );
      ...
      for (let i = 0; i < dirs.length; i += 1) {
        ...
        await this.#auroraFtpService.downloadAuroraGameDataDirectory(
          this.gameConsoleConfiguration,
          dirs[i],
        );
